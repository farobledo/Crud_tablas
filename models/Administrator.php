...
class Administrator extends db{
	...
	private function register_users($data){
		try {
			$SQL = 'INSERT INTO users (brand_user,last_product_user,city_user,price_user) VALUES (?,?,?,?)';
			$result = $this->connect()->prepare($SQL);
	...
	private function update_user($data){
		try {
			$SQL = 'UPDATE users SET brand_user = ?, last_product_user = ?, city_user = ?, price_user = ? WHERE id_user = ?';
			$result = $this->connect()->prepare($SQL);
			$result->execute(array(
				                  $data['brand'],
				                  $data['last_product'],
